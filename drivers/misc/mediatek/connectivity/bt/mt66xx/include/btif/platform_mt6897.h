...
static inline int32_t bgfsys_power_on(void)
{
	...
	BTMTK_INFO("bgfsys version id = 0x%08x (expected 0x%08x)", value, BGF_IP_VER_ID);
	if (value != BGF_IP_VER_ID && value != 0x02060000) {
		BTMTK_ERR("bgfsys version mismatch: 0x%08x != 0x%08x", value, BGF_IP_VER_ID);
		goto error;
	}
	...
	do {
		if (conninfra_reg_readable()) {
			...
			BTMTK_WARN("conninfra_reg_readable() returned 0 at retry=%d", retry);
			if (retry < (IDLE_LOOP_RTY_LMT - 10)) {
				if (base)
					iounmap(base);
				bgfsys_power_on_dump_cr();
				return -1;
