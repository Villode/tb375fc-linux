...
int consys_print_debug_mt6897(int level)
{
	if (level < 0 || level > 2) {
...
static int consys_check_conninfra_on_domain(void)
{
	...
	/* MT6897 ON domain check (matches vendor binary consys_check_conninfra_on_domain_status_mt6897):
	 * checks infracfg_ao offset 0x1cc bit 0 and offset 0x0c bit 25.
	 */
	r1 = CONSYS_REG_READ_BIT(CONN_REG_INFRACFG_AO_ADDR + 0x1cc, (0x1 << 0));
	r2 = CONSYS_REG_READ_BIT(CONN_REG_INFRACFG_AO_ADDR + 0x0c, (0x1 << 25));
	if (r1 || r2) {
		pr_info("%s: ON domain check failed: 0x1000_11cc[0]=0x%x, 0x1000_100c[25]=0x%x\n",
			__func__, r1, r2);
		return 0;
	}
	return 1;
...
static int consys_check_conninfra_off_domain(void)
{
	...
	if (r != 0x6) {
		pr_info("%s: bus clock detect failed (r=0x%x != 0x6)\n", __func__, r);
		return 0;
	}

	r = CONSYS_REG_READ(CONN_CFG_IP_VERSION_ADDR);
	if (r != CONN_HW_VER) {
		pr_info("%s: IP version check failed (r=0x%08x != 0x%08x)\n",
			__func__, r, CONN_HW_VER);
		return 0;
	}
...
static int __consys_check_reg_readable(int check_type)
{
	...
	if (consys_check_conninfra_on_domain() == 0) {
		pr_info("%s: on_domain check failed\n", __func__);
		consys_print_debug_mt6897(0);
		return 0;
	}

	if (consys_check_conninfra_off_domain() == 0) {
		pr_info("%s: check conninfra off failed\n", __func__);
		consys_print_debug_mt6897(1);
...
static void consys_debug_init_mt6897(void)
{
	debug_info = (struct conn_debug_info_mt6897 *)osal_malloc(sizeof(struct conn_debug_info_mt6897));
	if (debug_info == NULL) {
...
static void consys_debug_deinit_mt6897(void)
{
	if (debug_info != NULL) {
...
int consys_reg_init(struct platform_device *pdev)
{
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			base_addr = &conn_reg_mt6897.reg_base_addr[i];
			...
			pr_notice("XAGA-REGMAP: idx %d %-34s phy=0x%zx size=0x%zx\n",
				i, consys_base_addr_index_to_str[i],
				base_addr->phy_addr, base_addr->size);
		}
...
static int consys_reg_deinit(void)
{
	...
	for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
		if (conn_reg_mt6897.reg_base_addr[i].vir_addr) {
			pr_debug("[%d] Unmap %s (0x%zx)",
				i, consys_base_addr_index_to_str[i],
				conn_reg_mt6897.reg_base_addr[i].vir_addr);
			iounmap((void __iomem*)conn_reg_mt6897.reg_base_addr[i].vir_addr);
			conn_reg_mt6897.reg_base_addr[i].vir_addr = 0;
		}
	}

	consys_debug_deinit_mt6897();
