...
#include "mt6897_consys_reg.h"
#include "mt6897_consys_reg_offset.h"
#include "mt6897_connsyslog.h"
#include "clock_mng.h"
...
********************************************************************************
*/
static int consys_clk_get_from_dts_mt6897(struct platform_device *pdev);
static int consys_clock_buffer_ctrl_mt6897(unsigned int enable);
static unsigned int consys_soc_chipid_get_mt6897(void);
static void consys_clock_fail_dump_mt6897(void);
static unsigned int consys_get_hw_ver_mt6897(void);
static int consys_thermal_query_mt6897(void);
/* Power state relative */
static int consys_enable_power_dump_mt6897(void);
static int consys_reset_power_state_mt6897(void);
static int consys_reset_power_state(void);
static int consys_power_state_dump_mt6897(char *buf, unsigned int size);

static unsigned long long consys_soc_timestamp_get_mt6897(void);

static unsigned int consys_adie_detection_mt6897(void);
static void consys_set_mcu_control_mt6897(int type, bool onoff);

static int consys_pre_cal_backup_mt6897(unsigned int offset, unsigned int size);
static int consys_pre_cal_clean_data_mt6897(void);
/*******************************************************************************
*                            P U B L I C   D A T A
********************************************************************************
*/
...
struct consys_hw_ops_struct g_consys_hw_ops_mt6897 = {
	...
	/* clock */
	.consys_plt_clock_buffer_ctrl = consys_clock_buffer_ctrl_mt6897,
	.consys_plt_co_clock_type = consys_co_clock_type_mt6897,
	...
	/* debug */
	.consys_plt_clock_fail_dump = consys_clock_fail_dump_mt6897,
...
static struct consys_plat_thermal_data_mt6897 g_consys_plat_therm_data;
...
int consys_platform_spm_conn_ctrl_mt6897(unsigned int enable)
{
	int ret = 0;
...
int consys_clock_buffer_ctrl_mt6897(unsigned int enable)
{
...
void consys_clock_fail_dump_mt6897(void)
{
...
static void consys_print_irq_status(void)
{
	...
	if (consys_check_conninfra_on_domain_mt6897() == 0)
		return;
...
{
	memcpy(&g_consys_plat_therm_data, input, sizeof(struct consys_plat_thermal_data_mt6897));
}

static int calculate_thermal_temperature(int y)
{
	struct consys_plat_thermal_data_mt6897 *data = &g_consys_plat_therm_data;
	int t;
	int const_offset = 30;
...
#define CONN_GPT2_CTRL_AP_EN	0x38
	...
	connsys_adie_top_ck_en_ctl_mt6897(true);

	/* Hold Semaphore, TODO: may not need this, because
		thermal cr seperate for different  */
	if (consys_sema_acquire_timeout_mt6897(CONN_SEMA_THERMAL_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_notice("[THERM QRY] Require semaphore fail\n");
		connsys_adie_top_ck_en_ctl_mt6897(false);
		iounmap(addr);
		return -1;
	}
	...
	consys_sema_release_mt6897(CONN_SEMA_THERMAL_INDEX);
	connsys_adie_top_ck_en_ctl_mt6897(false);
...
int consys_pre_cal_restore_mt6897(void)
{
	...
	pr_info("[%s] mt6637_backup_cr_number=%d mt6637_backup_data=%x",
		__func__, mt6637_backup_cr_number, mt6637_backup_data);
	/* Acquire semaphore */
	if (consys_sema_acquire_timeout_mt6897(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
	 ...
	 */
	consys_spi_write_nolock_mt6897(SYS_SPI_TOP, 0xb04, 0xfe000000);
	consys_spi_write_nolock_mt6897(SYS_SPI_TOP, 0xb08, 0xe0000000);
	consys_spi_write_nolock_mt6897(SYS_SPI_TOP, 0xa04, 0xffffffff);
	consys_spi_write_nolock_mt6897(SYS_SPI_TOP, 0xaf4, 0xffffffff);
	/* Write CR back, SYS_SPI_WF & SYS_SPI_WF1 */
	for (i = 0; i < mt6637_backup_cr_number; i++) {
		consys_spi_write_nolock_mt6897(
			SYS_SPI_WF,
	 ...
	 */
	consys_spi_write_nolock_mt6897(SYS_SPI_TOP, 0xb04, 0x88000000);
	consys_spi_write_nolock_mt6897(SYS_SPI_TOP, 0xb08, 0x00000000);
	consys_spi_write_nolock_mt6897(SYS_SPI_TOP, 0xa04, 0x00000000);
	consys_spi_write_nolock_mt6897(SYS_SPI_TOP, 0xaf4, 0x00000000);
	/* Release semaphore */
