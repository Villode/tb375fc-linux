...
int consys_conninfra_on_power_ctrl_mt6897(unsigned int enable)
{
	...
	ret = consys_platform_spm_conn_ctrl_mt6897(enable);
#else
	ret = consys_conninfra_on_power_ctrl_mt6897_gen(enable);
#endif /* MTK_CONNINFRA_CLOCK_BUFFER_API_AVAILABLE */
	if (enable)
		consys_update_ap2conn_hclk_mt6897_gen();
...
void consys_set_if_pinmux_mt6897(unsigned int enable)
{
	...
	if (enable) {
		...
		if (clock_type == CONNSYS_CLOCK_SCHEMATIC_26M_EXTCXO ||
			...
			consys_set_gpio_tcxo_mode_mt6897_gen(1, 1);
	#else
		...
		if (clock_type == CONNSYS_CLOCK_SCHEMATIC_26M_EXTCXO ||
			clock_type == CONNSYS_CLOCK_SCHEMATIC_52M_EXTCXO) {
	#if defined(CFG_CONNINFRA_ON_CTP)
			consys_set_gpio_tcxo_mode_mt6897_gen(1, 0);
	#else
			if (!IS_ERR(g_conninfra_pinctrl_ptr)) {
...
int connsys_d_die_cfg_mt6897(void)
{
	...
	/* Vendor stock always writes 0 (normal ASIC mode) */
	CONSYS_REG_WRITE(CONN_INFRA_SYSRAM_SW_CR_BUILD_MODE, 0);
...
int consys_spi_read_nolock_mt6897(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	/* Read action:
...
int consys_spi_read_mt6897(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	...
	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS)
		return consys_spi_read_nolock_mt6897(subsystem, addr, data);
...
int consys_spi_write_nolock_mt6897(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
#ifndef CONFIG_FPGA_EARLY_PORTING
	int check = 0;
#endif
...
int consys_spi_write_mt6897(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	...
	if (subsystem == SYS_SPI_FM || subsystem == SYS_SPI_GPS)
		return consys_spi_write_nolock_mt6897(subsystem, addr, data);
...
int consys_spi_update_bits_mt6897(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data, unsigned int mask)
{
	int ret = 0;
...
int consys_subsys_status_update_mt6897(bool on, int radio)
{
	...
	if (consys_sema_acquire_timeout_mt6897(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
	...
	if (radio == CONNDRV_TYPE_FM) {
		if (consys_sema_acquire_timeout_mt6897(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_SUCCESS) {
