...
int connsys_a_die_switch_to_gpio_mode_mt6897_gen(void)
{
	...
	if ((consys_co_clock_type_mt6897() == CONNSYS_CLOCK_SCHEMATIC_26M_COTMS) ||
		(consys_co_clock_type_mt6897() == CONNSYS_CLOCK_SCHEMATIC_26M_EXTCXO)) {
		...
		#endif
	} else if ((consys_co_clock_type_mt6897() == CONNSYS_CLOCK_SCHEMATIC_52M_COTMS) ||
		(consys_co_clock_type_mt6897() == CONNSYS_CLOCK_SCHEMATIC_52M_EXTCXO)) {
	...
	/* read MT6637 ID */
		...
		if (check || (chip_id & 0xffff0000) != 0x66370000) {
			if (chip_id == 0 && consys_get_adie_chipid_mt6897() == ADIE_6637) {
				pr_info("[%s] A-die SPI read returned 0; using known MT6637 ID\n", __func__);
