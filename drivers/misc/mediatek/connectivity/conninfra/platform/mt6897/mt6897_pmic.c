...
#if 0
static int consys_plt_pmic_raise_voltage_mt6897(unsigned int, bool, bool);
static void consys_plt_pmic_raise_voltage_timer_handler_mt6897(timer_handler_arg);
#endif
...
#if 0
	.consys_pmic_raise_voltage = consys_plt_pmic_raise_voltage_mt6897,
#endif
...
#else
	int sleep_mode = consys_get_sleep_mode_mt6897();

	if (enable) {
		/* Mainline bring-up: the MT6363/MT6368 WiFi PMIC regulators are
		 * not wired into the DTS yet.  Guard every access so missing
		 * regulators don't crash consys power-on.
		 */
		if (reg_VRFIO18) {
			...
			pr_info("VRFIO18 regulator missing, skip\n");
		}
		...
		if (reg_VCN13) {
			...
			pr_info("VCN13 regulator missing, skip\n");
		}
	} else {
...
#define ATOP_DUMP_NUM 10
	...
	for (i = 0; i < ATOP_DUMP_NUM; i++) {
		consys_spi_read_mt6897(SYS_SPI_TOP, adie_cr_list[i], &adie_value);
